...
| `db.cosmosdb.user_agent` | string | Cosmos client SDK user agent. | `Cosmos-netstandard-sdk/3.31.2001P` | Recommended |
| `db.cosmosdb.connection_mode` | string | Cosmos client connection mode. | `gateway` | Required |
| `db.cosmosdb.container` | string | Cosmos DB container name. | `todo_items` | Conditionally Required: if available |
| `db.cosmosdb.request_content_length` | int | Request payload size in bytes |  | Recommended |
| `db.cosmosdb.response_content_length` | int | Response payload size in bytes |  | Recommended |
...
| `db.cosmosdb.regions_contacted` | string[] | Regional endpoints (URLs? host names?) contacted by any operation - TODO more specific [2] | `[westus2, westus3]` | Recommended |
| `db.cosmosdb.retry_count` | int | Number of retries [3] |  | Recommended |
| `db.cosmosdb.item_count` | int | Number of items written or read from the database |  | Conditionally Required: when operation involves more than one item. |
| `db.cosmosdb.activity_id` | string | Unique Id for the operation and can be helpful to debug particular operation in backend logs. [4] | `todo` | Recommended |
| `db.cosmosdb.correlated_activity_id` | string | It will be populated only in case of query operation to allow correlating query pages retrieved for the same multi-page or cross-partition query. [5] | `todo` | Recommended |

**[1]:** when response was received and contained sub-code.

**[2]:** TODO Is it needed? We have it on network-level calls AND in cosmos diagnostics

**[3]:** TODO - what does it mean if there were multiple network operations in scope of this logical call?

**[4]:** TODO This is .NET-specific and hopefully would be removed in future. I'd leave it un-speced.

**[5]:** TODO This is .NET-specific and hopefully would be removed in future. I'd leave it un-speced.
...
| `db.cosmosdb.user_agent` | `Cosmos-netstandard-sdk/3.31.2001P|1|X64|Microsoft Windows 10.0.17763|.NET 6.0.13|N|` |
